...
use bgi::backend::{Backend, DrawCommand, pixel_buffer::PixelBufferBackend};
use bgi::{color::RgbColor, types::GraphicsMode};
...
fn test_pixel_buffer_basic_operations() {
    ...
    backend
        .init()
        .expect("Pixel buffer backend initialization should succeed");

    // Create a window
    let mode = GraphicsMode::new(bgi::types::GraphicsDriver::Vga, 2);
    let window_id = backend
        .create_window(100, 100, Some("Test Window"), mode)
...
fn test_pixel_buffer_coordinates() {
    ...
    let mode = GraphicsMode::new(bgi::types::GraphicsDriver::Vga, 2);
    let window_id = backend
        .create_window(50, 50, Some("Coord Test"), mode)
        .expect("Window creation should succeed");
...
fn test_pixel_buffer_input() {
    ...
    let _window_id = backend
        .create_window(100, 100, Some("Input Test"), mode)
        .expect("Window creation should succeed");
    ...
    assert!(
        events.is_empty(),
        "Headless backend should not generate input events"
    );
}
...
fn test_pixel_buffer_colors() {
    ...
    let mode = GraphicsMode::new(bgi::types::GraphicsDriver::Vga, 2);
    let window_id = backend
        .create_window(160, 10, Some("Color Test"), mode)
        .expect("Window creation should succeed");
    ...
    backend
        .draw(window_id, &commands)
        .expect("Color drawing should succeed");
    backend.present(window_id).expect("Present should succeed");
}
...
fn test_pixel_buffer_performance() {
    ...
    let mode = GraphicsMode::new(bgi::types::GraphicsDriver::Vga, 2);
    let window_id = backend
        .create_window(200, 200, Some("Performance Test"), mode)
        .expect("Window creation should succeed");
    ...
    backend
        .draw(window_id, &commands)
        .expect("Performance drawing should succeed");
    backend.present(window_id).expect("Present should succeed");
    ...
    assert!(
        ...
        "40K pixel operations should complete in <100ms, took {:?}",
        elapsed
    );
}
...
fn test_pixel_buffer_multiple_windows() {
    ...
    let window1 = backend
        .create_window(100, 100, Some("Window 1"), mode)
        .expect("Window 1 creation should succeed");
    let window2 = backend
        .create_window(200, 150, Some("Window 2"), mode)
        .expect("Window 2 creation should succeed");

    // Draw to different windows
    let commands1 = vec![DrawCommand::Pixel {
        x: 50,
        y: 50,
        color: RgbColor {
            r: 255,
    ...
    let commands2 = vec![DrawCommand::Pixel {
        x: 100,
        y: 75,
        color: RgbColor {
            r: 0,
    ...
    backend
        .draw(window1, &commands1)
        .expect("Drawing to window 1 should succeed");
    backend
        .draw(window2, &commands2)
        .expect("Drawing to window 2 should succeed");

    backend
        .present(window1)
        .expect("Present window 1 should succeed");
    backend
        .present(window2)
        .expect("Present window 2 should succeed");
}
...
fn test_pixel_buffer_capabilities() {
    ...
    assert!(
        capabilities.multi_window,
        "Pixel buffer should support multiple windows"
    );
