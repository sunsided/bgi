...
fn test_bit_pattern_logic() {
    ...
    for counter in 0..16 {
        ...
        println!("counter={:2}, bit_pos={:2}, should_plot={}", counter, bit_position, should_plot);
    }

    // Count how many bits are set to 1
    let ones_count = (0..16).map(|counter| {
        let bit_position = 15 - (counter % 16);
        if (dotted_pattern >> bit_position) & 1 != 0 { 1 } else { 0 }
    }).sum::<i32>();
    ...
    println!("Expected pixels in 41-pixel dotted line: ~{}", expected_pixels);
