...
use bgi::{
    backend::{pixel_buffer::PixelBufferBackend, Backend, DrawCommand},
    color::RgbColor,
    types::{GraphicsMode, GraphicsDriver},
};
...
fn test_pixel_buffer_drawing_validation() {
    ...
    let pixel_cmd = DrawCommand::Pixel { x: 10, y: 10, color: red };
    backend.draw(window_id, &[pixel_cmd]).expect("Failed to draw pixel");
    
    // Verify the pixel was drawn correctly
    assert!(backend.verify_pixel(10, 10, red).expect("Failed to verify pixel"));
    println!("✓ Pixel validation successful");
    
    // Test 2: Draw a line and verify it
    let white = RgbColor::new(255, 255, 255);
    let line_cmd = DrawCommand::Line { x1: 0, y1: 0, x2: 10, y2: 10, color: white };
    backend.draw(window_id, &[line_cmd]).expect("Failed to draw line");
    
    // Verify line was drawn (check endpoints)
    assert!(backend.verify_line_drawn(0, 0, 10, 10, white).expect("Failed to verify line"));
    println!("✓ Line validation successful");
    
    // Test 3: Count drawn pixels
    let pixel_count = backend.count_drawn_pixels().expect("Failed to count pixels");
    assert!(pixel_count > 0, "Expected some pixels to be drawn, got: {}", pixel_count);
    ...
    let pixel_count = backend.count_drawn_pixels().expect("Failed to count pixels after clear");
    assert_eq!(pixel_count, 0, "Expected 0 pixels after clear, got: {}", pixel_count);
...
fn test_shape_drawing_validation() {
    ...
    let circle_cmd = DrawCommand::Circle { x: 50, y: 50, radius: 20, color: blue, filled: false };
    backend.draw(window_id, &[circle_cmd]).expect("Failed to draw circle");
    
    let pixel_count = backend.count_drawn_pixels().expect("Failed to count pixels");
    assert!(pixel_count > 50, "Circle should draw many pixels, got: {}", pixel_count);
    println!("✓ Circle drawing validation: {} pixels", pixel_count);

    // Test rectangle drawing
    let green = RgbColor::new(0, 255, 0);
    let rect_cmd = DrawCommand::Rectangle { x1: 10, y1: 10, x2: 90, y2: 90, color: green, filled: false };
    backend.draw(window_id, &[rect_cmd]).expect("Failed to draw rectangle");
    
    let pixel_count = backend.count_drawn_pixels().expect("Failed to count pixels");
    assert!(pixel_count > 100, "Rectangle should add more pixels, got: {}", pixel_count);
    println!("✓ Rectangle drawing validation: {} pixels total", pixel_count);

    // Test that we can get specific pixel colors
    // The rectangle should have green pixels at the corners
    assert!(backend.verify_pixel(10, 10, green).expect("Failed to verify rectangle corner"));
