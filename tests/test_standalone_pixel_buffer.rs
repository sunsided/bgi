...
fn test_standalone_pixel_buffer_validation() {
    ...
    let cleared_count = backend.count_drawn_pixels().expect("Failed to count pixels after clear");
    // Note: In real implementations, clear might not reduce pixel count to 0 if background is non-black
    println!("✓ Buffer clearing successful: {} pixels after clear", cleared_count);
...
fn test_shape_drawing_validation() {
    ...
    let circle_cmd = DrawCommand::Circle { x: 100, y: 100, radius: 50, color: blue, filled: false };
    backend.draw(window_id, &[circle_cmd]).expect("Failed to draw circle");
    
    // Test rectangle drawing  
    let green = RgbColor::new(0, 255, 0);
    let rect_cmd = DrawCommand::Rectangle { x1: 20, y1: 20, x2: 80, y2: 60, color: green, filled: false };
    backend.draw(window_id, &[rect_cmd]).expect("Failed to draw rectangle");
    
    // Verify drawing occurred
    let pixel_count = backend.count_drawn_pixels().expect("Failed to count pixels");
    assert!(pixel_count > 100, "Should have drawn many pixels for shapes, got: {}", pixel_count);
    println!("✓ Shape drawing validation successful: {} pixels drawn", pixel_count);
}
