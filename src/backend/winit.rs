...
impl WindowState {
    ...
    fn present(&mut self) -> BgiResult<()> {
        ...
        self.pixels
            .render()
            .map_err(|e| BgiError::GraphicsError(format!("Render error: {}", e)))?;
        Ok(())
    }
...
impl WinitBackend {
    ...
    fn color_to_u32(&self, color: Color) -> u32 {
        match color {
            ...
            Color::Rgb(rgb) => ((rgb.r as u32) << 16) | ((rgb.g as u32) << 8) | (rgb.b as u32),
        }
    }
}
...
impl Backend for WinitBackend {
    fn create_window(
        ...
        Err(BgiError::GraphicsError(
            "Cannot create windows without event loop access".to_string(),
        ))
    }
        ...
        F: FnMut(&mut dyn Backend, InputEvent) -> bool + 'static,
    {
        ...
        Err(BgiError::GraphicsError(
            "Event loop not implemented yet".to_string(),
        ))
    }
